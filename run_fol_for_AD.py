...
def main(args, visualize=False):
    ...
    for video_idx, track_file in enumerate(all_track_files):
        ...
        print(video_name)
        if os.path.isfile(os.path.join(args.save_dir, video_name ,video_name+'.pkl')):
        ...
        S_DIR = os.path.join(args.save_dir, video_name)
        if not os.path.isdir(S_DIR):
            os.mkdir(S_DIR)
        # load tracking data and ego motion data
        track_data = np.loadtxt(track_file, delimiter=',')
        ...
        for frame_id in range(2, current_frame_id): # 수정해야 할 수도 있음.
            '''
        ...
        #current_frame_id -= 1
        #video_len -= 2
        while current_frame_id + 1 < video_len  :#track_data[-1][0]:
